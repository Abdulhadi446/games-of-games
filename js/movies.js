...
async function loadData() {
  ...
  STATE.items = data.filter(item => item.type === "movie");
}
...
function renderItems() {
  ...
  if (shown.length === 0) {
    out.innerHTML = '<p class="meta">No movies found. Try a different filter or search.</p>';
    return;
  }
  out.innerHTML = "";
  shown.forEach(item => {
    ...
    card.innerHTML = `
      ...
      <div class="meta">Movie • ${escapeHtml((item.categories || []).join(", "))}</div>
...
function openModal(item) {
  ...
  els.modalContent.innerHTML = `
    ...
    <p class="meta">Movie • ${escapeHtml((item.categories || []).join(", "))}</p>
