...
async function loadData() {
  ...
  STATE.items = data.filter(item => item.type === "game");
}
...
function renderItems() {
  ...
  if (shown.length === 0) {
    out.innerHTML = '<p class="meta">No games found. Try a different filter or search.</p>';
    return;
  }
  out.innerHTML = "";
  shown.forEach(item => {
    ...
    card.innerHTML = `
      ...
      <div class="meta">Game • ${escapeHtml((item.categories || []).join(", "))}</div>
...
function openModal(item) {
  ...
  els.modalContent.innerHTML = `
    ...
    <p class="meta">Game • ${escapeHtml((item.categories || []).join(", "))}</p>
